...
| Sensitive files | Deny-by-default path patterns (.env*, keys, SSH, cloud creds, keychains…) are enforced at resolve time; `.c2cignore` adds workspace rules. `read_file` and both search engines also suppress otherwise allowed files containing credential-like values. |
